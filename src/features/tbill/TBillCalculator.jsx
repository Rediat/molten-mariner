...
const TBillCalculator = ({ toggleHelp, toggleSettings }) => {
    ...
    const handleCalculate = () => {
        ...
        if (mode === 'forward') {
            ...
            setResult(res);
            addToHistory('T-BILL', { faceValue, issueDate, tenure, discountRate, brokerageRate, mode: 'forward' }, res);
        } else {
            // Reverse: Total Consideration → Face Value
            const purchasePrice = totalBudget / (1 + brokerageRate / 100);
            const brokerage = totalBudget - purchasePrice;
            const computedFaceValue = purchasePrice * (1 + (discountRate / 100) * (tenure / 365));
            const discountAmount = computedFaceValue - purchasePrice;
            const netReturn = computedFaceValue - totalBudget;
            const effectiveYield = (netReturn / totalBudget) * (365 / tenure) * 100;
            ...
            const res = {
                ...
                purchasePrice,
                brokerage,
                totalConsideration: totalBudget,
                discountAmount,
                effectiveYield,
                netReturn
            };
            ...
            setResult(res);
            addToHistory('T-BILL', { totalBudget, issueDate, tenure, discountRate, brokerageRate, mode: 'reverse' }, res);
        }
    };
    ...
    return (
        ...
        <div className="space-y-1.5 flex-1 overflow-y-auto pr-1 scrollbar-hide">
            ...
            {mode === 'forward' ? (
                ...
                <div className="bg-neutral-800/40 rounded-xl p-2.5 border border-emerald-500/50 ring-1 ring-emerald-500/10">
                    <div className="flex justify-between items-center gap-2 min-w-0">
                        <div className="shrink-0">
                            <label className="text-sm font-bold text-emerald-400 block leading-tight text-left">Budget</label>
                            <span className="text-[9px] uppercase tracking-wider text-neutral-500 font-bold">Total Consideration</span>
                        </div>
                        <FormattedNumberInput
                            value={totalBudget}
                            onChange={(e) => setTotalBudget(parseFloat(e.target.value.replace(/,/g, '')) || 0)}
                            decimals={2}
        ...
        {result && (() => {
            ...
            return (
                <div className={`mt-2 bg-gradient-to-br ${isReverse ? 'from-emerald-900/30' : 'from-primary-900/30'} to-neutral-800/50 border ${isReverse ? 'border-emerald-500/30' : 'border-primary-500/30'} rounded-xl p-3 space-y-2`}>
                    ...
                    <div className={`bg-neutral-900/80 rounded-lg p-2.5 border ${isReverse ? 'border-emerald-500/30' : 'border-primary-500/30'}`}>
