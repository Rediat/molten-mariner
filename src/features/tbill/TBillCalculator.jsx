...
const TBillCalculator = ({
    ...
    const [bankSource, setBankSource] = useState('WEGAGEN'); // 'WEGAGEN' | 'GADAA' | 'CBE' | 'AWASH' | 'ETHIO_FIDELITY' | 'FIRST_ADDIS'
                    ...
                    <div className="flex justify-between items-center">
                        <div className="text-left">
                            ...
                            <div className="flex items-center gap-1.5">
                                <div className={`w-1.5 h-1.5 rounded-full ${bankSource === 'WEGAGEN' ? 'bg-primary-500 shadow-[0_0_8px_rgba(14,165,233,0.8)]' : bankSource === 'GADAA' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]' : bankSource === 'AWASH' ? 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.8)]' : bankSource === 'ETHIO_FIDELITY' ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.8)]' : bankSource === 'FIRST_ADDIS' ? 'bg-sky-500 shadow-[0_0_8px_rgba(14,165,233,0.8)]' : 'bg-violet-500 shadow-[0_0_8px_rgba(139,92,246,0.8)]'}`} />
                                <span className={`text-[11px] font-semibold uppercase tracking-wider ${bankSource === 'WEGAGEN' ? 'text-primary-400' : bankSource === 'GADAA' ? 'text-emerald-400' : bankSource === 'AWASH' ? 'text-amber-400' : bankSource === 'ETHIO_FIDELITY' ? 'text-rose-400' : bankSource === 'FIRST_ADDIS' ? 'text-sky-400' : 'text-violet-400'}`}>
                                    {bankSource === 'WEGAGEN' ? 'Wegagen Capital Investment Bank (WCIB)' : bankSource === 'GADAA' ? 'Gadaa Securities Dealer S.C' : bankSource === 'AWASH' ? 'Awash Capital Investment Bank' : bankSource === 'ETHIO_FIDELITY' ? 'Ethio Fidelity Securities S.C' : bankSource === 'FIRST_ADDIS' ? 'First Addis Investment Bank (FAB)' : 'CBE Capital Investment Bank'}
                                </span>
                            </div>
                        </div>
                        <ChevronDown 
                    ...
                    {showBankSelector && (
                        <div className="absolute left-0 right-0 bottom-full mb-1 z-[50] bg-neutral-900 border border-neutral-700 rounded-xl p-2 shadow-2xl animate-in fade-in slide-in-from-bottom-1 duration-200 ring-1 ring-white/10 max-h-[155px] overflow-y-auto scrollbar-hide">
                            <div className="grid grid-cols-1 gap-1">
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); setBankSource('FIRST_ADDIS'); setShowBankSelector(false); }}
                                    className={`flex items-center justify-between p-2 rounded-lg transition-all ${bankSource === 'FIRST_ADDIS' ? 'bg-sky-600/20 border border-sky-500/50' : 'hover:bg-neutral-800 border border-transparent'}`}
                                >
                                    <div className="flex items-center gap-2">
                                        <div className={`w-2 h-2 rounded-full ${bankSource === 'FIRST_ADDIS' ? 'bg-sky-500' : 'bg-neutral-700'}`} />
                                        <div className="text-left">
                                            <p className={`text-[11px] font-bold uppercase tracking-wider ${bankSource === 'FIRST_ADDIS' ? 'text-sky-400' : 'text-neutral-400'}`}>First Addis Investment Bank (FAB)</p>
                                            <p className="text-[8px] font-bold text-neutral-500 tracking-tighter">ET07FABIA0103028</p>
                                        </div>
                                    </div>
                                    {bankSource === 'FIRST_ADDIS' && <div className="w-1 h-4 bg-sky-500 rounded-full" />}
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="mt-1.5 mb-1.5 bg-gradient-to-br from-emerald-900/30 to-neutral-800/50 border border-emerald-500/30 rounded-xl p-2.5 space-y-2">
                        <div className="flex justify-between items-center mb-1">
                            <div className="flex items-center gap-2">
                                ...
                                <div className={`flex items-center gap-1.5 text-[8px] font-bold uppercase tracking-tight ${bankSource === 'WEGAGEN' ? 'text-primary-400' : bankSource === 'GADAA' ? 'text-emerald-400' : bankSource === 'AWASH' ? 'text-amber-400' : bankSource === 'ETHIO_FIDELITY' ? 'text-rose-400' : bankSource === 'FIRST_ADDIS' ? 'text-sky-400' : 'text-violet-400'}`}>
                                    <div className={`w-1 h-1 rounded-full ${bankSource === 'WEGAGEN' ? 'bg-primary-500' : bankSource === 'GADAA' ? 'bg-emerald-500' : bankSource === 'AWASH' ? 'bg-amber-500' : bankSource === 'ETHIO_FIDELITY' ? 'bg-rose-500' : bankSource === 'FIRST_ADDIS' ? 'bg-sky-500' : 'bg-violet-500'}`} />
                                    {bankSource === 'CBE' ? 'CBE Capital' : bankSource === 'AWASH' ? 'Awash Capital' : bankSource === 'ETHIO_FIDELITY' ? 'Ethio Fidelity' : bankSource === 'FIRST_ADDIS' ? 'First Addis' : bankSource} Account
                                </div>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => { const bankInfo = { WEGAGEN: { name: 'Wegagen Capital Investment Bank (WCIB)', account: 'ET81WEGC00141021' }, GADAA: { name: 'Gadaa Securities Dealer S.C', account: 'ET57GADS00110312' }, CBE: { name: 'CBE Capital Investment Bank', account: 'ET49CBEC00140965' }, AWASH: { name: 'Awash Capital Investment Bank', account: 'ET21AWCA00113140' }, ETHIO_FIDELITY: { name: 'Ethio Fidelity Securities S.C', account: 'ET75ETHF00131050' }, FIRST_ADDIS: { name: 'First Addis Investment Bank (FAB)', account: 'ET07FABIA0103028' } }[bankSource]; generateTBillApplicationPDF({ faceValue: result.faceValue, tenure: tenure, issueDate: auctionDate, yieldRate: discountRate, maturityDate: result.maturityDate }, { accountNo: bankInfo.account, bankName: '' }); }} className={`text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 transition-all ${bankSource === 'WEGAGEN' ? 'text-primary-400 hover:text-primary-300' : bankSource === 'GADAA' ? 'text-emerald-400 hover:text-emerald-300' : bankSource === 'AWASH' ? 'text-amber-400 hover:text-amber-300' : bankSource === 'ETHIO_FIDELITY' ? 'text-rose-400 hover:text-rose-300' : bankSource === 'FIRST_ADDIS' ? 'text-sky-400 hover:text-sky-300' : 'text-violet-400 hover:text-violet-300'}`} title="Download Application Form"><Download size={11} /> Application</button>
