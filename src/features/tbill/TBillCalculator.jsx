...
const TBillCalculator = ({ toggleHelp, toggleSettings }) => {
    ...
    const handleCalculate = () => {
        ...
        if (mode === 'forward') {
            ...
            const purchasePrice = fv / (1 + (disc / 100) * (tenure / 365));
            const brokerage = purchasePrice * (brok / 100);
            const totalConsideration = purchasePrice + brokerage;
            const discountAmount = fv - purchasePrice;
            const netReturn = fv - totalConsideration;
            ...
            setResult(res);
            addToHistory('T-BILL', { faceValue: fv, issueDate, tenure, discountRate: disc, brokerageRate: brok, mode: 'forward' }, res);
        } else {
            // Reverse: Total Consideration → Face Value
            const purchasePrice = budget / (1 + brok / 100);
            const brokerage = budget - purchasePrice;
            const computedFaceValue = purchasePrice * (1 + (disc / 100) * (tenure / 365));
            const discountAmount = computedFaceValue - purchasePrice;
            const netReturn = computedFaceValue - budget;
            const effectiveYield = (netReturn / budget) * (365 / tenure) * 100;
